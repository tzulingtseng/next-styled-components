...
const StyledInput = styled('input')(({ theme, errorMessage, disabled }) => ({
    ...
    border: `1px solid ${isEmpty(errorMessage) ? theme.colors.grey1 : theme.colors.danger}`,
    "&:active, &:focus": {
...
const Input = ({ label, errorMessage, value, required, onChange, placeholder, ...props
}) => {
    return (
        <Fragment>
            <FormGroup>
                <FormLabel required={required}>{label}</FormLabel>
                <StyledInput value={value} onChange={onChange} errorMessage={errorMessage} placeholder={placeholder} {...props}></StyledInput>
            </FormGroup>
